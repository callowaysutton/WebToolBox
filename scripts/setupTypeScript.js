...
function updateSvelteFiles() {
	[
		...
		{
			view: 'routes/tools/index',
			vars: [{ name: 'tools', type: '{ slug: string; title: string, html: any }[]' }],
			contextModule: [
				{
					js: '.then(r => r.json())',
				...
				{
					js: '.then(tools => {',
					ts: '.then((tools: { slug: string; title: string, html: any }[]) => {'
				}
			]
		},
		{
			view: 'routes/tools/[slug]',
			vars: [{ name: 'tool', type: '{ slug: string; title: string, html: any }' }]
		}
